...
```
SendMessage(to: "director", summary: "<5-10 word summary>", message: "<your report or questions>")
```
...
The framework below is MANDATORY, not advisory. When gathering requirements, present all relevant questions in one `SendMessage` to the Director, grouped by category. Provide a brief context line per category explaining why you are asking. The Director will batch them into `AskUserQuestion` relays for the user.
...
1. **Clarify**: Read the target codebase for context. Send clarifying questions to the Director via `SendMessage`. Do NOT create any file until this step is complete.
2. **Draft**: Create the document at the OUTPUT PATH you were given. Use the `design-doc` skill template. Omit optional sections unless needed. Report completion to the Director via `SendMessage`.
3. **Internal Quality Loop**: The Director will relay Reviewer feedback. Apply revisions. Report completion so the Director can re-route to the Reviewer. Repeat until the Reviewer approves.
4. **User Approval**: The Director presents the polished draft to the user. If the user returns COMMENT markers or verbal feedback, the Director will relay them. Return to step 1 (new questions) or step 2 (revisions) as appropriate, then re-enter the internal loop. Repeat until approved.
5. **Finalize**: When the Director signals user approval, update Status, verify implementation steps are actionable, and report "finalized" via `SendMessage`.
...
- Summarize what was changed in your `SendMessage` report to the Director
...
5. **Change summary**: Report to the Director via `SendMessage` what was changed, organized by section. Include the original COMMENT and what was done to resolve it.
