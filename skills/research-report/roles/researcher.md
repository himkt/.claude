...
- **Claim your task on start.** Your spawn prompt includes a `YOUR TASK ID`. Call `TaskUpdate(taskId: YOUR TASK ID, owner: "researcher-[NN]", status: "in_progress")` as your first action, before any search work — use the literal `researcher-[NN]` name your spawn prompt assigned (e.g., `researcher-01`). Mark it `completed` when the output file is written and your completion report has been sent.
